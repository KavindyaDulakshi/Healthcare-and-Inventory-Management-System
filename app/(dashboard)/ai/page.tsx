...
export default function AiAssistantPage() {
  const { medicines, appointments, billing } = useHealthcare();
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: "init-1",
      sender: "ai",
      text: "Hello! I am your clinical copilot. I can query inventory statuses, predict medicine demand forecasting, review billing statements, and check doctor availability. How can I help you today?",
  ...
  const generateAiResponse = (query: string): ChatMessage => {
    ...
    if (textLower.includes("expiring") || textLower.includes("expire")) {
      const expiringList = medicines.filter((m) => {
        ...
        const limit = new Date("2026-07-30"); // Metformin expiring soon!
        return exp <= limit;
      });
    ...
    // 2. Predict demand
    if (textLower.includes("demand") || textLower.includes("predict")) {
      return {
        id,
        sender: "ai",
        ...
        component: (
          <div className="flex flex-col gap-3 mt-3 border border-border bg-card rounded-xl p-4 text-xs w-full max-w-md">
            <span className="font-bold text-primary flex items-center gap-1.5">
              <Sparkles className="h-4 w-4 shrink-0" /> Demand Forecast (July 2026)
            </span>
            <p className="text-muted-foreground leading-normal">
              Based on the 6 scheduled cardiology consultations and 4 asthma walk-ins, I forecast a 25% surge in respiratory and cardiac tablet drawrates.
            </p>
            <div className="flex flex-col gap-2.5 border-t border-border pt-3">
              <div className="flex justify-between items-center">
                <span>Atorvastatin 20mg</span>
                <span className="font-bold text-secondary">Recommended: +150 units</span>
              </div>
              <div className="flex justify-between items-center">
                <span>Salbutamol Inhaler</span>
                <span className="font-bold text-danger">Critically Out of Stock (Reorder!)</span>
              </div>
            </div>
          </div>
        )
    ...
    if (textLower.includes("report") || textLower.includes("inventory report")) {
      ...
      return {
        ...
        component: (
          <div className="flex flex-col gap-3 mt-3 border border-border bg-card rounded-xl p-4 text-xs w-full max-w-md">
            ...
            <div className="grid grid-cols-2 gap-2 mt-1 pb-3 border-b border-border">
  ...
  const SUGGESTION_PROMPTS = [
    ...
    { text: "Predict next month's demand", icon: TrendingUp, color: "text-primary bg-primary/10" },
    { text: "Generate inventory report", icon: Brain, color: "text-accent bg-accent/10" },
    ...
    <div className="flex-1 flex gap-6 h-[calc(100vh-12rem)] relative overflow-hidden">
      ...
      <div className="flex-1 bg-card border border-border rounded-2xl flex flex-col justify-between overflow-hidden shadow-xs relative">
        ...
        <div className="p-4 border-b border-border/40 flex items-center justify-between bg-muted/10 shrink-0">
          ...
          <Badge variant="secondary" className="rounded-lg text-[10px]">GPT-4 Mock Model</Badge>
        </div>
